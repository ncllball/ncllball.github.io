/**
 * Sync program card status badges in the landing page with pd-programs.json
 * Usage: node Player Development/update-card-status.js
 */
const fs = require('fs');
...
function ensureManifest(){
  ...
  execFileSync(process.execPath, [path.join(PD_DIR,'build-pd-manifest.js')], { stdio: 'inherit' });
}
...
function main(){
  ...
  // Normalize cost lines for free programs: remove any parenthetical mentioning FREE
  html = html.replace(/(<li><strong>Cost:<\/strong>\s*\$0[^<]*?)(\s*\([^<]*free[^<]*\))/gi, '$1');
