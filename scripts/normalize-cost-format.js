 ...
 *
 * Scope
 * - Tag strip Cost list items: <li class="tag-item" data-tag="cost">...$60 / player...</li>
 * - Cost sections: <h2 id="cost">Cost</h2> followed by a <p>
 *
 * Rules
 * - Keep single spaces around the slash: " / "
 * - Convert "per" to "/" (e.g., "$60 per player" -> "$60 / player")
 * - Preserve trailing parenthetical notes (e.g., "(scholarships available)")
 * - Skip FREE cases (those are handled by normalize-free-cost.js)
 *
 * Usage
...
function normalizeCostText(text) {
  // Skip if mentions FREE (case-insensitive)
  if (/\bfree\b/i.test(text)) return { text, changed: false };

  let t = text;
  const original = t;

  // Normalize "per" to slash
  t = t.replace(/\bper\b/gi, '/');

  // Collapse spaces around slash and ensure single spaces: " / "
  // Capture amount and unit, keep any trailing text
  // Examples handled:
  //   $60/ player -> $60 / player
  //   $60 /player -> $60 / player
  //   $60 /  player -> $60 / player
  //   $60   per   player -> $60 / player
  //   $125.00 / team (scholarships) -> preserves (scholarships)
  t = t.replace(/\$(\d+(?:\.\d{2})?)\s*(?:\/)\s*([A-Za-z][A-Za-z &\-]*[A-Za-z]|[A-Za-z]+)([^<]*)/g,
    (_m, amt, unit, rest) => `$${amt} / ${unit}${rest}`);

  // If we had patterns like "$60 per session" without slash captured earlier
  t = t.replace(/\$(\d+(?:\.\d{2})?)\s*\/\s*/g, (_m, amt) => `$${amt} / `);
...
function normalizeTagStripCost(html) {
  let changed = false;
  let out = html;
  // Replace inner text following <strong>Cost:</strong>
  out = out.replace(/(<li\b[^>]*\bdata-tag=\"cost\"[^>]*>\s*<strong>\s*Cost:\s*<\/strong>)([\s\S]*?)(<\/li>)/gi,
    (full, open, inner, close) => {
      const { text, changed: c } = normalizeCostText(inner);
      if (c) changed = true;
...
function normalizeCostSection(html) {
  let changed = false;
  let out = html;
  // Only adjust the first paragraph following the Cost heading
  const re = /(<h2\b[^>]*id=\"cost\"[^>]*>\s*Cost\s*<\/h2>)([\s\S]*?)(<p\b[^>]*>)([\s\S]*?)(<\/p>)/gi;
  out = out.replace(re, (match, h2, between, pOpen, pInner, pClose) => {
    const { text, changed: c } = normalizeCostText(pInner);
    if (!c) return match;
    changed = true;
    return h2 + between + pOpen + text + pClose;
  });
  return { html: out, changed };
}
...
function main() {
  ...
  console.log(`${WRITE ? 'Applied' : 'Would apply'} cost spacing normalization in ${changedCount} file(s)`);
