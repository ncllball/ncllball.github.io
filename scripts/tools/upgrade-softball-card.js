...
const filePath = path.join(__dirname, '..', '..', 'Softball', 'index.html');
...
if (!match) {
  console.log('No legacy softball cards remain - all caught up!');
  process.exit(0);
}
...
const slugBase = `uniforms-softball-${baseToken}`;
...
const heroThumbMarkup = hasHero
  ? `                        <div class="ncll-card__graphics">\n                            <label for="lightbox-softball-${shortSlug}-1" class="lightbox-thumb">\n${indent(
      heroImgTag,
      32
    )}\n                            </label>\n                        </div>`
  : '';
...
const heroLightboxMarkup = hasHero
  ? `\n                <label for="lightbox-softball-${shortSlug}-1" class="lightbox" data-id="1" role="dialog" aria-modal="true"\n                    aria-label="Enlarged ${title} illustration" tabindex="0">\n                    <span>Close image</span>\n                    <span class="close-btn" aria-hidden="true">&times;</span>\n${indent(
      heroImgTag,
      20
    )}\n                </label>`
  : '';
...
const heroCheckboxLine = hasHero
  ? `                <input type="checkbox" id="lightbox-softball-${shortSlug}-1" class="lightbox-toggle">`
  : '';
